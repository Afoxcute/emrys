...
TOKENS_INFO = {
    ...
    "AVAX": {
        ...
        "decimals": 18,
        "is_native": True,
        "description": "Native token of the Avalanche network",
    },
    "MATIC": {
        "name": "Polygon",
        "symbol": "MATIC",
        ...
        "decimals": 18,
        "is_native": True,
        "description": "Native token of the Polygon network",
    },
    "BNB": {
        "name": "Binance Coin",
        "symbol": "BNB",
        ...
        "decimals": 18,
        "is_native": True,
        "description": "Native token of the Binance Smart Chain",
    },
    "SOL": {
        "name": "Solana",
        "symbol": "SOL",
    ...
    "USDC": {
        "name": "USD Coin",
        "symbol": "USDC",
        "chains": ["Ethereum", "Avalanche", "Polygon", "BSC", "Solana"],
        "decimals": 6,
        "is_native": False,
        "description": "Fully-backed stablecoin pegged to the US dollar",
    },
    "USDT": {
        "name": "Tether",
        "symbol": "USDT",
...
async def get_faq_response(question: str) -> tuple[str, list[str]]:
    """
    Get a response to an FAQ question
    Returns the answer and suggested follow-up questions
    """
    question_lower = question.lower()
    
    # Simple keyword matching
    if "hello" in question_lower or "hi" in question_lower:
        return "👋 Hello! How can I help you with Emrys today?", ["What is Emrys?", "Which chains are supported?"]
    
    if "thank" in question_lower:
        return "You're welcome! Let me know if you have any other questions about Emrys.", ["How do fees work?", "Is Emrys secure?"]
    ...
    for faq in FAQ_DATA:
        ...
        if any(word.lower() in q for word in question_lower.split() if len(word) > 3):
            relevant_faqs.append(faq)
    
    # If we found a matching FAQ, return it
    if relevant_faqs:
        faq = relevant_faqs[0]
        # Generate suggested questions (excluding the current one)
        suggested = [f["question"] for f in FAQ_DATA if f["question"] != faq["question"]][:3]
        return faq["answer"], suggested
    
    # Topic-based matching if no direct FAQ match
    if any(word in question_lower for word in ["bridge", "transfer", "send"]):
        return "Emrys makes it easy to bridge tokens between chains. Just click 'Start Bridging' at the top and select your preferred network.", ["What tokens can I bridge?", "How long do transfers take?"]
    
    if any(word in question_lower for word in ["walrus", "storage", "store"]):
        return "Emrys uses Walrus decentralized storage to securely store your transaction data across multiple networks. This ensures your data remains secure, immutable, and easily accessible regardless of which blockchain you're using.", ["How does Walrus work?", "Is Emrys secure?"]
    
    if any(word in question_lower for word in ["fetch", "agent", "uagent", "ai", "chat"]):
        return "This chat assistant is powered by fetch.ai uAgents technology, which provides intelligent, context-aware interactions. It allows me to understand your questions and provide relevant information about Emrys.", ["What is Emrys?", "What is Walrus storage?"]
    
    # Default response if no match
    return "I don't have specific information about that yet. You might want to check out our documentation or contact support for more details.", ["What is Emrys?", "Which chains are supported?", "How do fees work?"]

async def get_tokens_info() -> dict:
    """
